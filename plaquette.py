...
def MultiplyMatdagMatdag(left: np.ndarray, right: np.ndarray) -> np.ndarray:
    ...
    MM[0, 0] = np.conj(left[0, 0] * right[0, 0] + left[1, 0] * right[0, 1] + left[2, 0] * right[0, 2])
    MM[1, 0] = np.conj(left[0, 1] * right[0, 0] + left[1, 1] * right[0, 1] + left[2, 1] * right[0, 2])
    MM[2, 0] = np.conj(left[0, 2] * right[0, 0] + left[1, 2] * right[0, 1] + left[2, 2] * right[0, 2])
    # but take conjugate using np
    MM[0, 1] = np.conj(left[0, 0] * right[1, 0] + left[1, 0] * right[1, 1] + left[2, 0] * right[1, 2])
    MM[1, 1] = np.conj(left[0, 1] * right[1, 0] + left[1, 1] * right[1, 1] + left[2, 1] * right[1, 2])
    MM[2, 1] = np.conj(left[0, 2] * right[1, 0] + left[1, 2] * right[1, 1] + left[2, 2] * right[1, 2])
    # last index
    MM[0, 2] = np.conj(left[0, 0] * right[2, 0] + left[1, 0] * right[2, 1] + left[2, 0] * right[2, 2])
    MM[1, 2] = np.conj(left[0, 1] * right[2, 0] + left[1, 1] * right[2, 1] + left[2, 1] * right[2, 2])
    MM[2, 2] = np.conj(left[0, 2] * right[2, 0] + left[1, 2] * right[2, 1] + left[2, 2] * right[2, 2])
    return MM
...
def RealTraceMultMatMat(left: np.ndarray, right: np.ndarray) -> float:
    ...
    Tr(left*right)
    """
    TrMM = np.real(left[0, 0] * right[0, 0] + left[0, 1] * right[1, 0] + left[0, 2] * right[2, 0] +  # noqa: W504
                   left[1, 0] * right[0, 1] + left[1, 1] * right[1, 1] + left[1, 2] * right[2, 1] +  # noqa: W504
                    ...
                    for nz in range(0, shape[3]):
                        for nt in range(0, shape[0]):
                            ...
                            for cc, co in enumerate(coord):
                                if co >= shape[cc]:
                                    coord[cc] = 0
                            Umu_xnu = data[coord[0], coord[1], coord[2], coord[3], mu, :, :]
                            # U_nu(x)
                            coord = coordBase
